...
const CandidateProfile = () =>{
  ...
  const onSubmit: SubmitHandler<FormFields> = async (data) => {
    try {
      ...
      throw new Error("Error updating profile. Try again later.");
    } catch (e) {
      ...
      <header className="fixed top-0 w-full z-10">
        <div className="flex justify-between items-center bg-primary h-16 px-3 text-white">
          ...
          <div className="flex items-center md:mr-8">
            ...
            <div className="w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-white flex justify-center items-center bg-white text-primary font-semibold">H</div>
          </div>
        </div>
      </header>
        ...
        <div className="flex flex-col justify-center items-center md:w-3/5 w-full">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="w-full md:px-20 px-5">
            ...
            {...register("phone", {
              ...
              className={inputClassnames}
            />
            {errors.phone && <div className="mt-1 text-error text-sm text-left">{errors.phone.message}</div>}
